 ...
 *
 * The original class had several bugs and/or performance problems that were fixed.
 * The new AnnotationHelper can handle FigPolys, ie, lines with bend points.
 *
...
public class AnnotationHelper{
    ...
    public static final int getNormOffset(Point p0, Point p1, Point p2){
        ...
        float dd = (float)( ( (p2.x-p1.x)*(p0.y-p1.y) - (p0.x-p1.x)*(p2.y-p1.y) ) /
    ...
    public static final float getRatio(Point from, Point begin, Point end) {
        ...
        // begin + diffToStart*direction is the intersection of the perpendicular
        float diffToStart = directionX * (from.x-begin.x)
        +directionY * (from.y-begin.y);
        // dividing by length gives absolute offset, dividing twice gives relative
